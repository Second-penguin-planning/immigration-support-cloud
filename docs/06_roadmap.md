...
| Phase8 | 本番公開（Vercelデプロイ・バックアップ運用・復元テスト） | ✅ 完了（手順・スクリプト整備済み。実際のVercelデプロイ・復元テストはユーザー環境での実施が必要、下記参照） |
...
- 単体テスト: ストレージfacadeのドライバ切替ロジック（`FILE_STORAGE_DRIVER`未設定時はlocal、
  ...
  （`npm run test`）
- `npm run typecheck` / `npm run lint` / `npm run format:check` / `npm run build` は全て成功
- バックアップ・リストアスクリプトは、Docker停止時のフォールバック分岐（`DATABASE_URL`経由）が
  正しく動作すること、および引数不足時のエラーメッセージ表示を手動実行で確認した
  （実データに対する`pg_dump`/`pg_restore`の実行そのものは、ローカルにPostgresクライアントが
  無く、かつ`isc-postgres`コンテナも起動できない環境のため未検証）

### 既知の制約

- **実際のVercelデプロイは未実施**: 本セッションはユーザーのVercelアカウントを操作する権限・
  認証情報を持たないため、デプロイ手順の整備までとし、実行はユーザーが
  [07_deployment.md](./07_deployment.md)の手順に沿って行う
- **バックアップ・リストアの実データ検証は未実施**: 本セッション中Docker Desktopが
  最後まで復旧しなかった（Phase6・Phase7から継続する既知の制約）ため、
  `isc-postgres`コンテナに対する実際のバックアップ取得・リストアの往復確認ができていない
- **Phase6・Phase7のブラウザ動作確認も未実施のまま**: Docker環境が復旧し次第、
  AI補助・誤入力検知・定期届出の一連の画面操作を先行して確認すること
